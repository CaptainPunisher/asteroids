...
public:
	...
	void initXWindows(void)
	{
		...
		Colormap cmap = XCreateColormap(X11.dpy, root, vi->visual, AllocNone);
		swa.colormap = cmap;
...
void set_title(void)
{
	...
	XStoreName(X11.dpy, X11.win, "3350 Asteroids template");
}
...
void show_mouse_cursor(const int onoff)
{
	...
	Cursor cursor;
	//make a blank cursor
	blank = XCreateBitmapFromData (X11.dpy, X11.win, data, 1, 1);
	if (blank == None)
		std::cout << "error: out of memory." << std::endl;
	cursor = XCreatePixmapCursor(X11.dpy, blank, blank, &dummy, &dummy, 0, 0);
	XFreePixmap(X11.dpy, blank);
